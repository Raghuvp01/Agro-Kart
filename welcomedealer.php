	...
	if( isset($_REQUEST['submit']) )
	{
			...
			if( $flag==1 )
			{
			$sql="update dealers set name='$name',district='$district',state='$state' where mobno='$fid'";
			$res=execute( $sql );
			}
			else{
			$sql="insert into dealers(name,mobno,district,state) values('$name','$fid','$district','$state')";
			$res=execute( $sql );
			}
			
			echo "<br/><br/><center><img src='img/ok.png' class='img img-responsive' /><h4 style='color:blue'>Profile got edited</h4><a href=welcomedealer.php class='btn btn-danger'>Return</a></center>";
	...
	else
	{
			...
			$sql="select * from dealers where mobno='$fid'";
